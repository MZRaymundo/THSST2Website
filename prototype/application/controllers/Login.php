...
class Login extends CI_Controller {
    ...
    public function index(){
    	...
    	//dynamic css & js
		$data['css'] = link_tag('assets/css/login.css');
		$data['js'] = "<script type='text/javascript' src='<?php echo base_url(); ?>assets/js/login.js'></script>";
    ...
    public function check(){
		...
		if($this->form_validation->run() === FALSE){
			...
			if($this->login_model->check()) {
				...
				$data['invalidInput'] = "Invalid Email/Password";
				$this->load->view('pages/view_login', $data);
			}
		}
    }
